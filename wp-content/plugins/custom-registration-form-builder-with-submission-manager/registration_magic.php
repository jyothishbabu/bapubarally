...
define('RM_PLUGIN_VERSION', '3.5.2.2');
define('RM_DB_VERSION', 4.3);
...
function registration_magic_register_autoload() {
    ...
    function registration_magic_include_external_libs() {
        require_once RM_EXTERNAL_DIR . 'session/wp_session_utility.php';
    ...
    /* Initializing custom session handlers */
    if(!defined('WP_SESSION_COOKIE'))
            define( 'WP_SESSION_COOKIE', '_wp_session' );
    $session_utility= new RM_WP_Session_Utils();
    add_action( 'wp_session_garbage_collection', array($session_utility,'wp_session_cleanup') );
    add_action( 'wp', array($session_utility,'wp_session_register_garbage_collection') );
    $session_utility->wp_session_start();
    if ( ! defined( 'WP_CLI' ) || false === WP_CLI ) {
	add_action( 'shutdown', array($session_utility,'wp_session_write_close') );
    }
