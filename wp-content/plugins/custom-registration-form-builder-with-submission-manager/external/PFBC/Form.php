...
class Form extends Base
{
    ...
    {   
        global $rm_session;
        $errors = array();
        if ($rm_session instanceof RM_WP_Session && $rm_session->session_started())
        {
            $errors = array();
            $id = $this->_attributes["id"];
            if (!empty(self::$session["pfbc-".$id."-errors"])){
                
                if(self::$session["pfbc-".$id."-errors"] instanceof RM_Session_Recursive_ArrayAccess )
                    $errors = self::$session["pfbc-".$id."-errors"]->toArray();
                else
    ...
    {  
        ...
        $values = array();
        if (!empty(self::$session["pfbc-".$id."-values"]))
            $values = self::$session["pfbc-".$id."-values"]->toArray();
    ...
    {   
        ...
        if($valid)
        {
            $rm_session['rm_session'.$id]= array();
        }
    ...
    {   global $rm_session;
        self::$session=  $rm_session['rm_session'.$id];
...
JS;
    ...
    protected function save()
    {
        ...
        $rm_session['rm_session'.$this->form_slug]= self::$session;             
