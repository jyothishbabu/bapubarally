...
formStyleApp.controller('formStyleCtrl', function ($scope, $http) {
    ...
    $scope.saveStyles = function () {
        ...
        $http({
            method: 'POST',
            url: ajaxurl + "?action=rm_save_form_view_sett",
            data: JSON.stringify($scope.styles),
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined}
        }).success(function (data) {
            if (data.toString().trim() === 'saved' || data.toString().trim() === 'not saved')
                window.location = '?page=rm_form_sett_manage&rm_form_id=' + $scope.styles.form_id;
...
function msieversion() {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./))  // If Internet Explorer, return version number
    {
        ...
        console.log(version);
        if (version < 9) {
            alert("You are using older version of IE. Please update IE to latest version or");
