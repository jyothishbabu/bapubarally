...
function verysimplestart_hex2rgba($color, $opacity = false) {
...
//Dynamic styles
function verysimplestart_custom_styles($custom) {
	...
	if ( (get_theme_mod('front_header_type','slider') == 'nothing' && is_front_page()) || (get_theme_mod('site_header_type') == 'nothing' && !is_front_page()) ) {
		...
		$rgba 	= verysimplestart_hex2rgba($menu_bg_color, 0.9);
	...
	wp_add_inline_style( 'verysimplestart-style', $custom );	
}
add_action( 'wp_enqueue_scripts', 'verysimplestart_custom_styles' );